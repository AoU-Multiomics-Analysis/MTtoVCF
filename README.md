...
When running through `main.wdl`, the exported VCF is always indexed. If `MakeDosage` is enabled, the workflow also emits `<FullPrefix>.dose.tsv.gz` and its `.tbi` index. If `MakePlink` is enabled, it emits `<FullPrefix>.pgen`, `<FullPrefix>.pvar`, and `<FullPrefix>.psam`. If `MakeLoFCarriers` is enabled, `VATHailTable` must be supplied and the workflow emits `<FullPrefix>.lof_carriers.HC.tsv.bgz` and `<FullPrefix>.lof_carriers.HC_or_LC.tsv.bgz`.
...
carriers. In `main.wdl`, this optional output is generated by a Hail-native task
...
`has_lof_variant`, `n_lof_variants`, `variant_ids`, and `lof_classes`.
