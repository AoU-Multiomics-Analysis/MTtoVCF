...
**Output:** `main.wdl` emits `PathVCF`, `PathAnnotations`, optional `TranscriptAnnotations`, `VCFIndex`, and `AnnotationIndex`. Both `.tbi` files are uploaded beside their sources in `OutputBucket`. Each indexing task requests `max(IndexMinDiskGiB, ceil(input GiB × IndexDiskMultiplier) + IndexDiskOverheadGiB)`.
