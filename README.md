...
| `MakeLoFCarriers` | If `true`, emit long-format sample-gene LoF carrier tables using the input MatrixTable and VAT Hail Table directly (default: `false`; `main.wdl` only; requires `VATHailTable`) |
...
When running through `main.wdl`, the exported VCF is always indexed. If `MakeDosage` is enabled, the workflow also emits `<FullPrefix>.dose.tsv.gz` and its `.tbi` index. If `MakePlink` is enabled, it emits `<FullPrefix>.pgen`, `<FullPrefix>.pvar`, and `<FullPrefix>.psam`. If `MakeLoFCarriers` is enabled, `VATHailTable` must be supplied and the workflow emits `<FullPrefix>.lof_variants.vcf.bgz`, its `.tbi` index, `<FullPrefix>.lof_carriers.HC.tsv.bgz`, and `<FullPrefix>.lof_carriers.HC_or_LC.tsv.bgz`.
...
non-reference LoF variant. HC-only uses VAT rows with `LoF == "HC"`; HC-or-LC
uses `LoF` in `{"HC", "LC"}`. Genotypes with any non-reference allele count as
carriers. In `main.wdl`, this optional output is generated by a Hail-native subworkflow
that reads the input MatrixTable and VAT Hail Table directly, applies the same
sample, BED, biallelic, PASS, AC, and AN filters as the VCF export, joins LoF
annotations by parsed VAT `vid`, and then aggregates carriers by sample and
gene. Output columns are `sample_id`, `gene_id`, `gene_symbol`,
`has_lof_variant`, `n_lof_variants`, `variant_ids`, and `lof_classes`. The
LoF VCF is exported from the same materialized filtered Hail data and contains
flat `LOF_GENE_ID`, `LOF_GENE_SYMBOL`, and `LOF_CLASS` INFO arrays. Its tabix
index is created afterward by a separate `bcftools index` task; carrier tables
are derived from the Hail intermediate rather than by parsing the VCF.
...
post hoc runs where a VCF and transcript annotations already exist.
...
This utility workflow creates the same sparse long-format LoF carrier tables
